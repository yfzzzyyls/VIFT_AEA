#!/usr/bin/env python3
"""
FIXED: Training script that converts absolute poses to relative poses with proper coordinate transformation.
...
def convert_absolute_to_relative(poses):
    """
    FIXED: Convert absolute poses to relative poses with proper coordinate transformation.
    
    Args:
        poses: [seq_len, 7] absolute poses (translation + quaternion)
    
    Returns:
        relative_poses: [seq_len, 7] relative poses in local frame
    """
    ...
    for i in range(1, seq_len):
        ...
        # Compute relative translation in WORLD coordinates
        trans_diff_world = curr_trans - prev_trans
        
        # CRITICAL FIX: Transform translation to previous frame's coordinate system
        ...
        # Store relative pose in local coordinates
...
class RelativePoseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if np.linalg.norm(relative_poses[0, :3]) > 1e-6 or np.linalg.norm(relative_poses[0, 3:] - [0, 0, 0, 1]) > 1e-6:
            # Old format - convert to relative with FIXED transformation
...
class FirstBatchLogger(Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        if trainer.current_epoch == 0 and batch_idx == 0:
            ...
            console.print(f"\n    Frame-to-frame translations (in LOCAL frame):")
...
def train_with_relative_poses(
    ...
    console.rule(f"[bold cyan]🚀 Training with FIXED Relative Poses[/bold cyan]")
    console.print(f"\nConfiguration:")
    console.print(f"  Pose scale: {pose_scale} (meters → centimeters)")
    console.print(f"  Pose format: Relative with proper coordinate transformation")
    console.print(f"  Learning rate: {learning_rate}")
    console.print(f"  Batch size: {batch_size}")
    console.print(f"\n[bold yellow]⚠️  CRITICAL FIX: Translations now in LOCAL frame coordinates[/bold yellow]")
    ...
    # Create datasets
    console.print("\n[bold]Loading data with FIXED relative pose conversion...[/bold]")
    ...
    # Verify data
    console.print("\n[bold]Verifying FIXED relative pose conversion...[/bold]")
    ...
    console.print(f"    Frame 1: {first_poses[1]} (relative movement in LOCAL frame)")
    ...
    tb_logger = TensorBoardLogger(
        save_dir="logs",
        name="pretrained_relative_fixed",
        version=f"scale_{pose_scale}_fixed"
    )
    ...
    if use_wandb:
        wandb_logger = WandbLogger(
            ...
            name=f"relative_poses_fixed_scale_{pose_scale}",
            tags=["pretrained", "relative-poses-fixed", f"scale_{pose_scale}"]
        )
    ...
    callbacks = [
        ModelCheckpoint(
            ...
            mode="min",
            save_top_k=3,
            dirpath=f"logs/checkpoints_lite_fixed_scale_{pose_scale}",
    ...
    # Train
    console.rule("[bold cyan]🏃 Starting Training with FIXED Relative Poses[/bold cyan]")
    console.print("\n[bold yellow]Key improvements:[/bold yellow]")
    console.print("  • Fixed coordinate transformation for translations")
    console.print("  • Translations now in LOCAL frame (not world frame)")
    console.print("  • Should match original VIFT data format")
    console.print("  • Expected to achieve <0.1° rotation error")
    console.print()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train with FIXED relative poses')
    ...
    console.print("[bold magenta]Training with FIXED Relative Poses[/bold magenta]")
    console.print("Converting absolute world coordinates to relative LOCAL frame poses\n")
