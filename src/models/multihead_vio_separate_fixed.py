"""
Multi-Head VIO Architecture with Separate Visual and IMU Processing - Fixed Version
This version includes proper quaternion initialization and handling.
"""
...
class RotationSpecializedHead(nn.Module):
    ...
    def __init__(
        ...
        # Initialize output layer to produce identity quaternions
        nn.init.zeros_(self.rotation_output.weight)
        nn.init.constant_(self.rotation_output.bias, 0.0)
        # Set bias for W component to 1.0 (identity quaternion is [0,0,0,1])
        self.rotation_output.bias.data[0] = 1.0  # Assuming WXYZ format initially
...
class MultiHeadVIOModelSeparate(L.LightningModule):
    """
    Multi-head VIO model with separate visual and IMU processing - Fixed version
    """
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        # Update metrics - using proper quaternion MAE
        with torch.no_grad():
