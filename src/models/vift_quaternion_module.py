...
from ..metrics.arvr_loss_wrapper_balanced import BalancedARVRLossWrapper


class VIFTQuaternionModel(L.LightningModule):
    """
    VIFT model modified to output quaternions instead of Euler angles.
    Uses the same transformer architecture but outputs 7D poses.
    """
    ...
    def __init__(
        ...
        # Use balanced loss function to prevent rotation from dominating
        # This scales rotation loss by 0.1x to match translation loss magnitude
        self.loss_fn = BalancedARVRLossWrapper(
            rotation_scale=0.1,
        ...
        # For tracking rotation MAE (in degrees)
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        Forward pass
        Args:
            batch: Dictionary with 'visual_features', 'imu_features', 'relative_poses'
        Returns:
            Predicted poses [B, seq_len, 7] with quaternions
        """
        # Extract separate features
        visual_features = batch.get('visual_features', None)  # [B, seq_len, 512]
        imu_features = batch.get('imu_features', None)  # [B, seq_len, 256]
        ...
        features = torch.cat([visual_features, imu_features], dim=-1)  # [B, seq_len, 768]
        
        # Create dummy batch tuple for compatibility with original interface
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        # Get predictions
        predictions = self(batch)  # [B, seq_len, 7]
        
        # Get ground truth - already in quaternion format from dataset
        target = batch.get('relative_poses', batch.get('poses'))  # [B, seq_len, 7] - tx,ty,tz,qx,qy,qz,qw
        ...
        # Compute losses using ARVRLossWrapper
        ...
        # Weight losses
        weighted_rot_loss = loss_dict['rotation_loss'] * self.hparams.rotation_weight
        weighted_trans_loss = loss_dict['translation_loss'] * self.hparams.translation_weight
        
        # Compute correct total loss with weights
        total_loss = weighted_rot_loss + weighted_trans_loss + loss_dict.get('regularization_loss', 0)
        
        # Log losses
        self.log('train/total_loss', total_loss, prog_bar=True)
        self.log('train/trans_loss', weighted_trans_loss)
        self.log('train/rot_loss', weighted_rot_loss)
    ...
    def validation_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        # Get ground truth - already in quaternion format
        target = batch.get('relative_poses', batch.get('poses'))  # [B, seq_len, 7]
        ...
        # Log losses with proper synchronization
        self.log('val/total_loss', total_loss, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
        self.log('val/trans_loss', weighted_trans_loss, on_step=False, on_epoch=True, sync_dist=True)
        self.log('val/rot_loss', weighted_rot_loss, on_step=False, on_epoch=True, sync_dist=True)
        
        # Update metrics
        with torch.no_grad():
            # Ensure tensors are contiguous for metrics
            ...
            self.log('val/trans_mae_cm', self.val_trans_mae, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
            self.log('val/rot_mae_deg', self.val_rot_mae, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
    ...
    def configure_optimizers(self):
        ...
        return {
            ...
            'lr_scheduler': {
                'scheduler': scheduler,
                'interval': 'step' if self.hparams.scheduler_type == 'onecycle' else 'epoch',
                'frequency': 1
            }
        }